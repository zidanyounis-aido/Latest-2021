...
{
    public partial class DiwanMaster : System.Web.UI.MasterPage
    {
        public Int32 userID;
        ...
        private void fillTree()
        {
            ...
            {
                companyName = "Incoming and outgoing";
                folderName = "fldrName";
            }
            else
            {
                companyName = "الوارد والصادر";
                folderName = "fldrNameAr";
            }

            treeDT = c.getUserFolders(userID, true);
            ...
            foreach (DataRow R in DR)
            {
                ...
                if (oldCompany != companyName.ToString())
                {
                    companyNode = new TreeNode();
                    oldCompany = companyName.ToString();
                ...
                node.NavigateUrl = "../Screen/DefaultDiwan.aspx?typeId=" + FolderIndex.ToString();
        ...
        protected void trvFolders_TreeNodePopulate(object sender, TreeNodeEventArgs e)
        {
            ...
            foreach (DataRow R in treeDT.Select("fldrParentID=" + e.Node.Value))
            {
                ...
                node.NavigateUrl = "../Screen/documentsList.aspx?isDiwan=1&fldrID=" + c.encrypt(R["fldrID"].ToString());
